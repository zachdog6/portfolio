...
app.post("/api/employee", (req, res) => {
    ...
    || (!user.hasOwnProperty("username")) || (!user.hasOwnProperty("password"))) {
        ...
        query.usernameInUse(user.username, (usernameInUse) => {
            if (usernameInUse) {
...
app.put("/api/employee/:id", (req, res) => {
    ...
    || (!user.hasOwnProperty("username")) || (!user.hasOwnProperty("password"))) {
        ...
        query.usernameInUseNotMatchingId(user.username, req.params.id, (usernameInUse) => {
            if (usernameInUse) {
                res.status(400).send("User with given username already exists");
            }
            else {
                query.idCheck(res, req.params.id, (idFound) => {
                    if(!idFound){
